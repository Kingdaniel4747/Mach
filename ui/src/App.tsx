...
import questFile from "./quests.json";

type Screen = "home" | "alarms" | "quests" | "todos" | "blocker" | "settings" | "qr";
type Challenge = "shake" | "snake" | "hold" | "qr" | "nfc";
type StandbyWidget = "clock" | "alarm" | "calendar" | "sleep" | "todos" | "focus";
...
type DailyQuest = { id: string; title: string; detail: string; category: string; difficulty: QuestDifficulty; durationMinutes: number; xp: number };
type ActiveQuest = { id: string; date: string; startedAt: number; endsAt: number; packages: string[] };
type QuestProgress = { enabled: boolean; difficulty: QuestDifficulty | "mixed"; completedDates: string[]; xp: number; rewardMinutesAvailable: number; active?: ActiveQuest };
type Alarm = {
  id: string;
  time: string;
  date: string;
...
type PermissionState = { notifications: boolean; exact: boolean; fullScreen: boolean; camera: boolean; dnd: boolean };
...
declare global {
  interface Window {
    ...
    machHandleBack?: () => boolean;
  }
}

const STORAGE_KEY = "mach-local-v3";
const SCHEMA_KEY = "mach-schema";
const LEGACY_STORAGE_KEY = "wachwerk-local-v3";
const LEGACY_SCHEMA_KEY = "wachwerk-schema";
...
const questPortfolio = (questFile.quests as DailyQuest[]).filter(quest =>
  typeof quest.id === "string" && typeof quest.title === "string" && typeof quest.detail === "string"
  && ["easy", "medium", "hard"].includes(quest.difficulty) && Number.isInteger(quest.durationMinutes)
  && quest.durationMinutes >= 1 && quest.durationMinutes <= 1_440 && Number.isFinite(quest.xp) && quest.xp >= 0,
);
...
function QRCode({ seed = "wachwerk", matrix }: { seed?: string; matrix?: { size: number; bits: string } | null }) {
  ...
  return <div className="qr-code" style={{ gridTemplateColumns: `repeat(${size}, 1fr)` }} aria-label="Persönlicher MACH-QR-Code">{Array.from({ length: size * size }, (_, index) => {
...
export default function App() {
  ...
  const [blocker, setBlocker] = useState<BlockerState>({ methods: {}, hasPasswords: {}, enabled: false, limitsEnabled: false, windowsEnabled: false, packages: [], nfcToken: "", method: "nfc", qrToken: "wachwerk-personal-code", limits: {}, windows: {}, hasPassword: false, limitReminderEnabled: false, limitReminderMinutes: 10, rewardMinutesAvailable: 0, questUntil: 0, questPackages: [] });
  ...
  const applyNativeState = (raw?: string | null) => {
    if (!raw) return;
    try {
      const state = JSON.parse(raw) as { completedOneTimeIds?: string[]; morningBlock?: {until: number; packages: string[]}; alarmRinging?: boolean };
      if (state.morningBlock) setMorningBlock(state.morningBlock);
      setAlarmRinging(Boolean(state.alarmRinging));
      if (Array.isArray(state.completedOneTimeIds)) setNativeCompleted(state.completedOneTimeIds);
    } catch { /* A malformed platform state must never block the local UI. */ }
  };

  useEffect(() => {
    // Static screens only need a fresh minute. Reserve one-second updates for
    // visible countdowns so the complete WebView UI is not reconciled constantly.
    const needsSecondTicks = focusState.active || morningBlock.until > Date.now() || Boolean(questProgress.active && questProgress.active.endsAt > Date.now());
    const refresh = () => { if (!document.hidden) setNow(new Date()); };
    const timer = window.setInterval(refresh, needsSecondTicks ? 1_000 : 30_000);
    document.addEventListener("visibilitychange", refresh);
    return () => { window.clearInterval(timer); document.removeEventListener("visibilitychange", refresh); };
  }, [focusState.active, morningBlock.until, questProgress.active?.endsAt]);
  useEffect(() => {
    try {
      if (window.localStorage.getItem(SCHEMA_KEY) !== "3") {
        // Do not erase the complete origin while migrating MACH-owned data.
        window.localStorage.setItem(SCHEMA_KEY, "3");
      }
      const saved = window.localStorage.getItem(STORAGE_KEY) ?? window.localStorage.getItem(LEGACY_STORAGE_KEY);
      if (saved) {
        try {
          const data = JSON.parse(saved) as Partial<{ alarms: Alarm[]; todos: Todo[]; habits: Habit[]; questProgress: QuestProgress; settings: Settings; blocker: BlockerState; standby: { clock: "digital" | "analog"; font?: "apple" | "soft" | "mono"; tone: "blue" | "amber" | "mint" | "rose"; left: StandbyWidget; right: StandbyWidget } }>;
          ...
          setTodos((data.todos ?? []).map(todo => ({ ...todo, reminderAt: todo.reminderAt ?? "", completedAt: todo.completedAt ?? (todo.done ? todo.createdAt : undefined) }))); setHabits((data.habits ?? []).map(habit => ({ ...habit, completedDates: habit.completedDates ?? [], missedDates: habit.missedDates ?? [] }))); setQuestProgress(current => ({ ...current, ...(data.questProgress ?? {}) }));
          if (data.blocker) setBlocker(current => ({ ...current, ...data.blocker, limits: data.blocker!.limits ?? {}, windows: data.blocker!.windows ?? {}, questUntil: data.blocker!.questUntil ?? 0, questPackages: data.blocker!.questPackages ?? [] }));
          if (data.standby) {
          ...
          if (!window.localStorage.getItem(STORAGE_KEY)) window.localStorage.setItem(STORAGE_KEY, saved);
          window.localStorage.removeItem(LEGACY_STORAGE_KEY);
          window.localStorage.removeItem(LEGACY_SCHEMA_KEY);
        } catch {
          window.localStorage.removeItem(STORAGE_KEY);
        }
      }
      const wakeKey = window.MachAndroid?.getWakeKeyState?.();
      ...
      applyNativeState(window.MachAndroid?.getNativeState?.());
      const nativeBlocker = window.MachAndroid?.getBlockerState?.();
  ...
  useEffect(() => {
    ...
    window.addEventListener("mach-native-state", listener);
    return () => window.removeEventListener("mach-native-state", listener);
  }, []);
  useEffect(() => {
    ...
    window.addEventListener("mach-custom-sound", listener);
    return () => window.removeEventListener("mach-custom-sound", listener);
  }, []);
  useEffect(() => {
    const listener = (event: Event) => {
    ...
    window.addEventListener("mach-nfc-enrolled", listener);
    return () => window.removeEventListener("mach-nfc-enrolled", listener);
  }, []);
  useEffect(() => {
    const listener = () => setSettings(current => ({ ...current, qrVerified: true }));
    window.addEventListener("mach-qr-verified", listener);
    return () => window.removeEventListener("mach-qr-verified", listener);
  }, []);
  useEffect(() => {
    const listener = (event: Event) => {
    ...
    window.addEventListener("mach-blocker-state", listener);
    return () => window.removeEventListener("mach-blocker-state", listener);
  }, []);
  useEffect(() => {
    const listener = (event: Event) => setFocusState(current => ({ ...current, ...(event as CustomEvent<Partial<FocusState>>).detail }));
    window.addEventListener("mach-focus-state", listener);
    return () => window.removeEventListener("mach-focus-state", listener);
  }, []);
  useEffect(() => {
    const listener = (event: Event) => setPermissions(current => ({ ...current, ...(event as CustomEvent<Partial<PermissionState>>).detail }));
    window.addEventListener("mach-permission-state", listener);
    return () => window.removeEventListener("mach-permission-state", listener);
  }, []);
  useEffect(() => {
    const listener = (event: Event) => setInstalledApps((event as CustomEvent<InstalledApp[]>).detail ?? []);
    window.addEventListener("mach-installed-apps", listener);
    return () => window.removeEventListener("mach-installed-apps", listener);
  }, []);
  useEffect(() => {
    const listener = (event: Event) => {
    ...
    window.addEventListener("mach-accessibility-state", listener);
    return () => window.removeEventListener("mach-accessibility-state", listener);
  }, []);
  useEffect(() => {
    const listener = (event: Event) => setSoundPreviewing(Boolean((event as CustomEvent<{ playing?: boolean }>).detail?.playing));
    window.addEventListener("mach-sound-preview", listener);
    return () => window.removeEventListener("mach-sound-preview", listener);
  }, []);
  useEffect(() => {
    const listener = (event: Event) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mach-open-screen", listener);
    return () => window.removeEventListener("mach-open-screen", listener);
  }, []);
  useEffect(() => {
    if (!hydrated) return;
  ...
  useEffect(() => {
    ...
    const persist = () => window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ alarms, todos, habits, questProgress, settings, blocker: blockerWithRewards, standby: { clock: standbyClock, font: standbyFont, tone: standbyTone, left: standbyLeft, right: standbyRight } }));
  ...
  }, [hydrated, alarms, todos, habits, questProgress, settings, blocker, standbyClock, standbyFont, standbyTone, standbyLeft, standbyRight]);
  useEffect(() => {
    if (!hydrated) return;
    try { window.MachAndroid?.syncAlarms(JSON.stringify(alarms)); } catch { /* Browser preview */ }
  }, [hydrated, alarms]);
  useEffect(() => {
    if (!hydrated) return;
    try { window.MachAndroid?.syncBedtime(settings.remindersOn, settings.bedtime, settings.reminderInterval, settings.reminderMinimumInterval, settings.sleepDetectMinutes, settings.nagMode, settings.reminderMessage); } catch { /* Browser preview */ }
  }, [hydrated, settings.remindersOn, settings.bedtime, settings.reminderInterval, settings.reminderMinimumInterval, settings.sleepDetectMinutes, settings.nagMode, settings.reminderMessage]);
  useEffect(() => {
    if (!hydrated) return;
    try { window.MachAndroid?.syncTodos?.(JSON.stringify(todos)); } catch { /* Browser preview */ }
  }, [hydrated, todos]);
  useEffect(() => {
    if (!hydrated) return;
    try { window.MachAndroid?.syncSettings?.(JSON.stringify(settings)); } catch { /* Browser preview */ }
  }, [hydrated, settings]);
  useEffect(() => {
    if (!hydrated) return;
    const blockerWithRewards = { ...blocker, rewardMinutesAvailable: questProgress.rewardMinutesAvailable };
    try { window.MachAndroid?.syncAppBlocker?.(JSON.stringify(blockerWithRewards)); } catch { /* Browser preview */ }
  }, [hydrated, blocker, questProgress.rewardMinutesAvailable]);
  useEffect(() => {
    const handleBack = () => {
      if (document.querySelector('[data-sheet-kind="number"]')) {
        document.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape", bubbles: true }));
        return true;
      }
      if (alarmDraft) { closeSheet("alarm"); return true; }
      if (todoModal) { closeSheet("todo"); return true; }
      if (chooseTodo) { closeSheet("choose"); return true; }
      if (screen === "settings") { closeSheet("settings"); return true; }
      if (nightMode) { exitStandby(); return true; }
      if (screen !== "home") { setScreen("home"); return true; }
      return false;
    };
    window.machHandleBack = handleBack;
    return () => { if (window.machHandleBack === handleBack) delete window.machHandleBack; };
  }, [alarmDraft, todoModal, chooseTodo, screen, nightMode, sheetClosing]);
  ...
  useEffect(() => {
    ...
    try {
      const matrix = window.MachAndroid?.getQrMatrix?.(token);
      if (matrix) setQrMatrix(JSON.parse(matrix) as { size: number; bits: string });
    } catch { setQrMatrix(null); }
  }, [screen, alarmDraft?.challenge]);
  useEffect(() => {
    if (screen !== "blocker" && screen !== "quests" && screen !== "settings") return;
    try {
      if (!installedApps.length) {
        const apps = window.MachAndroid?.getInstalledApps?.();
        if (apps) setInstalledApps(JSON.parse(apps) as InstalledApp[]);
      }
      setAccessibilityEnabled(window.MachAndroid?.isAccessibilityEnabled?.() ?? false);
      setUsageAccessEnabled(window.MachAndroid?.hasUsageAccess?.() ?? false);
    } catch { setInstalledApps([]); }
    const refreshUsage = () => {
      try {
        const usage = window.MachAndroid?.getAppUsage?.();
  ...
  useEffect(() => {
    if (!focusState.active || !window.MachAndroid?.getFocusTimerState) return;
    const refresh = () => {
      try {
        const next = JSON.parse(window.MachAndroid!.getFocusTimerState!()) as FocusState;
  ...
  const filteredApps = useMemo(() => {
    const query = appSearch.trim().toLocaleLowerCase("de-DE");
    return installedApps.filter(app => `${app.label} ${app.packageName}`.toLocaleLowerCase("de-DE").includes(query));
  }, [installedApps, appSearch]);
  const filteredFocusApps = useMemo(() => {
    const query = focusAppSearch.trim().toLocaleLowerCase("de-DE");
    return installedApps.filter(app => `${app.label} ${app.packageName}`.toLocaleLowerCase("de-DE").includes(query));
  }, [installedApps, focusAppSearch]);
  ...
  useEffect(() => {
    if (questProgress.active && now.getTime() >= questProgress.active.endsAt) finishDailyQuest();
  }, [now, questProgress.active?.endsAt]);
  ...
  function chooseAlarmSound() {
    if (window.MachAndroid?.chooseAlarmSound) window.MachAndroid.chooseAlarmSound();
    else showToast("Eigene Audiodateien können in der APK ausgewählt werden");
  }
  function previewAlarmSound() {
    if (soundPreviewing) {
      window.MachAndroid?.stopAlarmSoundPreview?.();
      setSoundPreviewing(false);
      showToast("Tonvorschau beendet");
      return;
    }
    if (window.MachAndroid?.previewAlarmSound) window.MachAndroid.previewAlarmSound(settings.sound);
  ...
  function createBackup() {
    ...
    const state = { alarms: safeAlarms, todos, habits, questProgress, settings: safeSettings, blocker: safeBlocker, standby: { clock: standbyClock, font: standbyFont, tone: standbyTone, left: standbyLeft, right: standbyRight } };
    const backup = JSON.stringify({ format: "mach-backup", version: 1, exportedAt: new Date().toISOString(), state });
    if (window.MachAndroid?.createBackup) window.MachAndroid.createBackup(backup);
    else {
  ...
  function chooseBackup() {
    if (window.MachAndroid?.chooseBackup) window.MachAndroid.chooseBackup();
  ...
  function enrollNfc(target: "alarm" | "blocker") {
    ...
    if (window.MachAndroid?.enrollNfcTag) window.MachAndroid.enrollNfcTag(target);
  ...
  function toggleBlockerWithKey() {
    ...
    if (!blocker[flag] && !accessibilityEnabled && window.MachAndroid) { window.MachAndroid.openAccessibilitySettings?.(); showToast("Erlaube zuerst den App-Blocker-Zugriff"); return; }
    if (scope === "limits" && !usageAccessEnabled && window.MachAndroid) { window.MachAndroid.openUsageAccessSettings?.(); return; }
    ...
    if ((blocker.methods[blockerTab] ?? blocker.method) === "nfc") {
      if (window.MachAndroid?.scanBlockerTag) window.MachAndroid.scanBlockerTag(blocker.nfcToken, scope);
      else setBlocker(current => ({ ...current, [flag]: !current[flag] }));
      return;
    }
    if ((blocker.methods[blockerTab] ?? blocker.method) === "qr") {
      if (window.MachAndroid?.scanBlockerQr) window.MachAndroid.scanBlockerQr(blocker.qrToken, scope);
      else setBlocker(current => ({ ...current, [flag]: !current[flag] }));
      return;
    }
    ...
    const accepted = window.MachAndroid?.toggleBlockerPassword
      ? window.MachAndroid.toggleBlockerPassword(blockerPassword, scope) : true;
  ...
  function saveBlockerPassword() {
    ...
    const saved = window.MachAndroid?.setBlockerPassword ? window.MachAndroid.setBlockerPassword(blockerPassword, blockerTab) : true;
  ...
  function startFocusTimer() {
    if (settings.focusPackages.length && !accessibilityEnabled) { window.MachAndroid?.openAccessibilitySettings?.(); showToast("Aktiviere zuerst den App-Blocker-Zugriff"); return; }
    if (settings.focusSilenceNotifications && !permissions.dnd) { window.MachAndroid?.openNotificationPolicySettings?.(); showToast("Erlaube zuerst den Nicht-stören-Zugriff"); return; }
    window.MachAndroid?.startFocusTimer?.(settings.focusWorkMinutes, settings.focusBreakMinutes, settings.focusRounds, JSON.stringify(settings.focusPackages), settings.focusSilenceNotifications);
  ...
  function cancelFocusTimer() {
    window.MachAndroid?.cancelFocusTimer?.();
  ...
  function saveAlarm(event: FormEvent) {
    ...
    if (saved.bedtimeReminderEnabled) {
      ...
      if (startImmediately) window.MachAndroid?.startBedtimeNow?.(saved.id);
    }
  ...
  function finishDailyQuest() {
    const active = questProgress.active;
    if (!active || active.date !== todayKey || Date.now() < active.endsAt || questDoneToday) return;
    const quest = questPortfolio.find(item => item.id === active.id) ?? dailyQuest;
    const reward = rewardProgress(questProgress.xp, quest.xp, 100, settings.questRewardMinutes, questProgress.rewardMinutesAvailable, settings.questRewardCap);
    setQuestProgress(current => ({ ...current, active: undefined, completedDates: [...current.completedDates.filter(date => date !== todayKey), todayKey], xp: reward.xp, rewardMinutesAvailable: reward.availableMinutes }));
    setBlocker(current => ({ ...current, questUntil: 0, questPackages: [], rewardMinutesAvailable: reward.availableMinutes }));
    showToast(reward.earnedMinutes ? `Quest geschafft · +${quest.xp} XP · +${reward.earnedMinutes} Bonusminuten` : `Quest geschafft · +${quest.xp} XP`);
  }
  function startDailyQuest() {
    if (!dailyQuest || questDoneToday) return;
    if (questProgress.active?.date === todayKey && questProgress.active.endsAt > Date.now()) return;
    if (!settings.questPackages.length) { showToast("Wähle zuerst Apps für Tagesquests in den Einstellungen aus"); setScreen("settings"); return; }
    if (!accessibilityEnabled && window.MachAndroid) { window.MachAndroid.openAccessibilitySettings?.(); showToast("Erlaube zuerst den App-Blocker-Zugriff"); return; }
    const startedAt = Date.now();
    const active: ActiveQuest = { id: dailyQuest.id, date: todayKey, startedAt, endsAt: startedAt + dailyQuest.durationMinutes * 60_000, packages: settings.questPackages };
    setQuestProgress(current => ({ ...current, active }));
    setBlocker(current => ({ ...current, questUntil: active.endsAt, questPackages: active.packages }));
    showToast(`${active.packages.length} Apps sind bis zum Ablauf der Quest gesperrt`);
  }
  ...
  function enterStandby() { setStandbyEditing(null); setNightMode(true); window.MachAndroid?.enterStandby?.(); }
  function exitStandby() { setStandbyEditing(null); setNightMode(false); window.MachAndroid?.exitStandby?.(); }
  ...
  function renderQuests() {
    ...
    return <><ScreenHeader eyebrow="QUEST" title="Tagesquest" />
      {!questProgress.enabled ? <section className="settings-card"><h3>Tagesquests sind ausgeschaltet</h3><p>Aktiviere sie in den Einstellungen, um täglich eine Quest zu erhalten.</p></section> : <section className={`daily-quest ${questDoneToday ? "complete" : ""}`}>
        ...
        <div className="quest-stats"><span>⏱ {dailyQuest.durationMinutes} Minuten</span><span>🔒 {settings.questPackages.length} Apps ausgewählt</span></div>
        {active ? <div className="quest-running"><strong>{formatCountdown(remaining)}</strong><span>Die Quest läuft. Die ausgewählten Apps bleiben bis zum Ablauf gesperrt.</span></div> : questDoneToday ? null : <button type="button" className="quest-complete" onClick={startDailyQuest}>Quest starten & Apps sperren</button>}
      </section>}
      <section className="settings-card"><span className="overline">FORTSCHRITT</span><div className="quest-level"><span>Level {questLevel}</span><div><i style={{ width: `${questLevelProgress}%` }} /></div><strong>{questLevelProgress}/100 XP</strong></div><div className="quest-stats"><span>🔥 {questStreak} Tage</span><span>🎟 {questProgress.rewardMinutesAvailable} Bonusminuten</span></div></section>
    </>;
  }
  ...
  function renderFocusTimer() {
    const focusApps = filteredFocusApps;
    return <section className={`focus-timer-card ${focusState.active ? "active" : ""}`}>
      ...
      {focusState.active ? <>
        ...
        {settings.focusSilenceNotifications && !permissions.dnd && <button type="button" className="permission-button" onClick={() => window.MachAndroid?.openNotificationPolicySettings?.()}>Nicht-stören-Zugriff erlauben</button>}
  ...
  function renderAlarms() {
    return <><ScreenHeader eyebrow="AUFSTEHEN" title="Deine Wecker" action="＋" onAction={() => openNewAlarm()} />
      <section className="alarm-cycle-panel"><div className="cycle-card quick-cycle-card"><span className="overline">SCHNELLPLANER</span><h3>{cycleMode === "wake" ? "Wann willst du aufstehen?" : "Du gehst jetzt schlafen"}</h3><div className="choice-row"><button type="button" className={cycleMode === "wake" ? "active" : ""} onClick={() => setCycleMode("wake")}>Aufstehen um</button><button type="button" className={cycleMode === "sleep" ? "active" : ""} onClick={() => setCycleMode("sleep")}>Ich schlafe jetzt</button></div>{cycleMode === "wake" ? <TimePicker label="Gewünschte Aufstehzeit" value={cycleTime} onChange={setCycleTime} /> : <div className="current-sleep-time"><strong>{pad(now.getHours())}:{pad(now.getMinutes())}</strong><span>aktuelle Uhrzeit · nicht veränderbar</span></div>}<p className="science-note">Orientierung mit {settings.cycleMinutes} Minuten pro Zyklus plus {settings.fallAsleepMinutes} Minuten Einschlafzeit. Beides kannst du in den Einstellungen ändern.</p></div>
        <div className="quick-suggestions"><div className="quick-suggestion-title"><strong>{cycleMode === "wake" ? "Passende Einschlafzeiten" : "Passende Weckzeiten"}</strong><span>4–6 Zyklen</span></div><div className="suggestion-list">{cycleSuggestions.map(item => <button type="button" className={selectedCycle === item.cycles ? "selected" : ""} key={item.cycles} onClick={() => selectCycle(item.cycles)}><div><strong>{item.time}</strong><span>{item.cycles} Zyklen · {formatDuration(item.cycles * settings.cycleMinutes)}</span></div><em>{item.cycles === 5 ? "empfohlen" : `${item.cycles} Zyklen`}</em></button>)}</div><button type="button" className="cycle-apply" onClick={applyCycle}>{cycleMode === "wake" ? `Wecker für ${cycleTime} erstellen` : "Daraus Wecker erstellen"}</button></div></section>
      {renderFocusTimer()}
  ...
  function renderSettings() {
    ...
    return <><ScreenHeader eyebrow="MACH." title="Einstellungen" /><section className="settings-card"><label className="field-label" htmlFor="profile-name">Dein Name</label><input id="profile-name" type="text" placeholder="Optional" value={settings.name} onChange={event => setSettings(current => ({ ...current, name: event.target.value }))} /><label className="field-label" htmlFor="app-font">Schrift in der App</label><select id="app-font" value={settings.appFont} onChange={event => setSettings(current => ({ ...current, appFont: event.target.value as Settings["appFont"] }))}><option value="modern">Modern</option><option value="rounded">Weich & rund</option><option value="classic">Klassisch</option></select><label className="field-label">Farbpalette</label><div className="palette-options" role="group" aria-label="Farbpalette">{([{ id: "classic", name: "Original", colors: ["#06131f", "#c9dcf8", "#9bf5b1", "#ffd347"] }, { id: "solar", name: "Sonnenwärme", colors: ["#003049", "#d62828", "#f77f00", "#fcbf49", "#eae2b7"] }, { id: "dusk", name: "Abendruhe", colors: ["#191629", "#c2b2f1", "#90cdb7", "#edd9bc"] }] as const).map(palette => <button type="button" key={palette.id} aria-pressed={settings.palette === palette.id} className={settings.palette === palette.id ? "selected" : ""} onClick={() => setSettings(current => ({ ...current, palette: palette.id }))}><span className="palette-swatches">{palette.colors.map(color => <i key={color} style={{ background: color }} />)}</span><strong>{palette.name}</strong><span className="palette-check">{settings.palette === palette.id ? "✓" : ""}</span></button>)}</div></section>
      ...
      <section className="settings-card"><span className="overline">TAGESQUEST · APPS SPERREN</span><p className="field-help">Diese Apps bleiben während einer laufenden Tagesquest gesperrt. Die Sperre endet nur automatisch nach Ablauf der Quest.</p><details className="quest-app-picker"><summary><span><strong>Sperr-Apps auswählen</strong><small>{settings.questPackages.length ? `${settings.questPackages.length} ausgewählt` : "Noch keine Apps ausgewählt"}</small></span><b>⌄</b></summary><div className="quest-app-actions"><button type="button" onClick={() => setSettings(current => ({ ...current, questPackages: installedApps.map(app => app.packageName) }))}>Alle auswählen</button><button type="button" onClick={() => setSettings(current => ({ ...current, questPackages: [] }))}>Alle abwählen</button></div><label className="app-search"><span>⌕</span><input type="search" aria-label="Quest-Apps durchsuchen" placeholder="Apps suchen …" value={appSearch} onChange={event => setAppSearch(event.target.value)} /></label><div className="quest-app-list">{filteredApps.map(app => { const selected = settings.questPackages.includes(app.packageName); return <button type="button" key={app.packageName} className={selected ? "selected" : ""} onClick={() => setSettings(current => ({ ...current, questPackages: selected ? current.questPackages.filter(pkg => pkg !== app.packageName) : [...current.questPackages, app.packageName] }))}><AppIcon app={app} compact /><strong>{app.label}</strong><i>{selected ? "✓" : ""}</i></button>; })}{!filteredApps.length && <div className="empty-state"><span>⌕</span><h3>{appSearch ? "Keine passenden Apps" : "Apps werden vorbereitet"}</h3></div>}</div></details></section>
      ...
      <section className="settings-card backup-card"><span className="overline">BACKUP & WIEDERHERSTELLUNG</span><h3>Fortschritt mitnehmen</h3><p className="field-help">Exportiert Wecker, Einstellungen, Aufgaben, Habits, Quests und Blocker-Regeln als lokale JSON-Datei. Android-Berechtigungen, Passwörter, eigene Audiodateien und NFC-Hardwarezugriffe werden aus Sicherheitsgründen nicht kopiert.</p><div className="dual-actions"><button type="button" className="primary-button" onClick={createBackup}>Backup speichern</button><button type="button" className="secondary-button" onClick={chooseBackup}>Backup einspielen</button></div></section>
      ...
      {(!permissions.notifications || !permissions.exact || !permissions.fullScreen || !permissions.camera) && <section className="settings-card"><span className="overline">NOCH OFFENE BERECHTIGUNGEN</span>{!permissions.notifications && <div className="switch-row"><div><strong>Benachrichtigungen erlauben</strong><small>Für Wecker, Timer und Erinnerungen</small></div><button type="button" className="secondary-button compact" onClick={() => window.MachAndroid?.openNotificationSettings?.()}>Öffnen</button></div>}{!permissions.exact && <div className="switch-row"><div><strong>Alarme & Erinnerungen</strong><small>Damit Wecker sekundengenau klingeln</small></div><button type="button" className="secondary-button compact" onClick={() => window.MachAndroid?.openExactAlarmSettings?.()}>Öffnen</button></div>}{!permissions.fullScreen && <div className="switch-row"><div><strong>Vollbild-Wecker</strong><small>Damit der ausgeschaltete Bildschirm aufwacht</small></div><button type="button" className="secondary-button compact" onClick={() => window.MachAndroid?.openFullScreenSettings?.()}>Öffnen</button></div>}{!permissions.camera && <div className="switch-row"><div><strong>Kamera für QR-Aufgabe</strong><small>Wird ausschließlich beim Scannen benutzt</small></div><button type="button" className="secondary-button compact" onClick={() => window.MachAndroid?.requestCameraPermission?.()}>Erlauben</button></div>}<p className="field-help">Sobald eine Berechtigung erteilt ist, verschwindet ihr Hinweis automatisch.</p></section>}
    ...
    return <>{renderBlockerHeader()}
      ...
      <section className="settings-card morning-config"><span className="overline">NACH DEM AUFSTEHEN</span>
        <div className="switch-row"><div><strong>Handy-Pause am Morgen</strong><small>Startet nach dem Ausschalten eines Weckers</small></div><Toggle on={settings.morningBlockEnabled} label="Morgensperre aktivieren" onClick={() => {
          ...
          if (!settings.morningBlockEnabled && !accessibilityEnabled && window.MachAndroid) { window.MachAndroid.openAccessibilitySettings?.(); showToast("Aktiviere zuerst den App-Blocker-Zugriff"); return; }
      ...
      {!accessibilityEnabled && <section className="settings-card"><h3>App-Blocker-Zugriff</h3><button type="button" className="secondary-button" onClick={() => window.MachAndroid?.openAccessibilitySettings?.()}>Bedienungshilfe öffnen</button></section>}
  ...
  function renderBlocker() {
    ...
    const accessCard = needsAccessibility || needsUsage ? <section className="settings-card blocker-access-card"><span className="overline">NOCH ERFORDERLICHER ANDROID-ZUGRIFF</span>{needsAccessibility && <><div className="switch-row"><div><strong>App-Blocker aktivieren</strong><small>Erkennt, welche App gerade geöffnet wird</small></div><i className="status-dot" /></div><button type="button" className="secondary-button" onClick={() => window.MachAndroid?.openAccessibilitySettings?.()}>Bedienungshilfe öffnen</button></>}{needsUsage && <><div className="switch-row permission-row"><div><strong>Genaue Nutzungszeit erlauben</strong><small>Notwendig für korrekte Tageslimits</small></div><i className="status-dot" /></div><button type="button" className="secondary-button" onClick={() => window.MachAndroid?.openUsageAccessSettings?.()}>Nutzungsdatenzugriff öffnen</button></>}<p className="field-help">Sobald alles erlaubt ist, verschwindet dieser Hinweis automatisch. Alle Messwerte bleiben lokal.</p></section> : null;
  ...
  function renderAlarmModal() {
    ...
    return <div className={`modal-backdrop ${sheetClosing ? "closing" : ""}`} role="presentation" onPointerDown={event => { if (event.target === event.currentTarget) closeSheet("alarm"); }}>
      <form data-sheet-kind="alarm" className={`modal-sheet alarm-modal-sheet ${sheetClosing ? "closing" : ""}`} role="dialog" aria-modal="true" aria-label="Wecker bearbeiten" onSubmit={saveAlarm}>
        ...
        {alarmDraft.challenge === "qr" && <section className="qr-alarm-preview qr-inline-card"><div><strong>Dein persönlicher QR-Code</strong><small>Dieser Code bleibt gleich. Prüfe einmal, dass dein Ausdruck scanbar ist, bevor du ihn als Weckschlüssel verwendest.</small></div><QRCode seed="wachwerk-personal-code" matrix={qrMatrix} />{!settings.qrVerified ? <button type="button" className="secondary-button" onClick={() => window.MachAndroid?.verifyAlarmQr?.()}>Ausdruck einmal prüfen</button> : <small>✓ QR-Code schon erfolgreich geprüft</small>}<button type="button" className="file-picker-button" onClick={() => { if (window.MachAndroid?.printCurrentPage) window.MachAndroid.printCurrentPage(); else window.print(); }}>Als PDF drucken</button></section>}
  ...
  return <div className="app-shell"><main className={`app-root app-font-${settings.appFont}`}><div className={`app-screen screen-${screen}`} ref={scrollRef}>{alarmRinging && <button className="ringing-banner" type="button" onClick={() => window.MachAndroid?.openRingingAlarm?.()}><span>◷</span><span><strong>Dein Wecker klingelt</strong><small>Zur Aufwachaufgabe</small></span></button>}{(screen === "home" || screen === "settings") && renderHome()}{screen === "alarms" && renderAlarms()}{screen === "quests" && renderQuests()}{screen === "todos" && renderTodos()}{screen === "blocker" && renderBlocker()}{screen === "qr" && renderQR()}</div><nav className="bottom-nav">{navItems.map(item => <button type="button" key={item.id} className={screen === item.id ? "active" : ""} onClick={() => setScreen(item.id)}><span>{item.icon}</span><small>{item.label}</small></button>)}</nav>{renderAlarmModal()}{renderTodoModal()}{renderExtraSheets()}{toast && <div className="toast">{toast}</div>}</main></div>;
}
