...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lastExactPermission = AlarmScheduler.canScheduleExact(this);
    ...
    private void buildWebView() {
        ...
        webSettings.setUserAgentString(webSettings.getUserAgentString() + " MachAndroid/1.13-Offline");

        webView.addJavascriptInterface(new MachBridge(), "MachAndroid");
    ...
    protected void onResume() {
        ...
        boolean exactPermission = AlarmScheduler.canScheduleExact(this);
        if (!schedulesRestored || (!lastExactPermission && exactPermission)) {
            AlarmScheduler.restoreAll(this);
            TodoReminderScheduler.restore(this);
            FocusTimerScheduler.restore(this);
            schedulesRestored = true;
        }
        lastExactPermission = exactPermission;
        dispatchNativeState();
        dispatchBlockerState();
        dispatchFocusState();
        dispatchPermissionState();
    ...
    private void dispatchBlockerState() {
        if (!pageReady || webView == null) return;
        String script = "window.dispatchEvent(new CustomEvent('mach-blocker-state',{detail:" + AppBlockerStore.stateJson(this) + "}));"
            + "window.dispatchEvent(new CustomEvent('mach-accessibility-state',{detail:{enabled:" + isBlockerAccessibilityEnabled()
    ...
    private void dispatchFocusState() {
        if (!pageReady || webView == null) return;
        String script = "window.dispatchEvent(new CustomEvent('mach-focus-state',{detail:"
    ...
    private void dispatchPermissionState() {
        if (!pageReady || webView == null) return;
        String script = "window.dispatchEvent(new CustomEvent('mach-permission-state',{detail:"
            + permissionStateJson() + "}));";
        handler.post(() -> webView.evaluateJavascript(script, null));
    }
    ...
    public void onBackPressed() {
        if (pageReady && webView != null) {
            webView.evaluateJavascript("(function(){try{return !!(window.machHandleBack&&window.machHandleBack())}catch(e){return false}})()",
                handled -> { if (!"true".equals(handled)) performDefaultBack(); });
        } else {
            performDefaultBack();
        }
    }
    ...
    public final class MachBridge {
        @JavascriptInterface
        public void syncAlarms(String json) {
            AlarmScheduler.syncAlarms(getApplicationContext(), json);
            if (json != null && json.contains("\"enabled\":true")) handler.post(MainActivity.this::requestNotificationPermission);
        ...
        public void syncTodos(String json) {
            ...
            if (json != null && json.matches("(?s).*\\\"reminderAt\\\":\\\"[^\\\"]+.*")) handler.post(MainActivity.this::requestNotificationPermission);
    ...
    private void dispatchSoundPreviewState(boolean playing) {
        if (!pageReady || webView == null) return;
        String script = "window.dispatchEvent(new CustomEvent('mach-sound-preview',{detail:{playing:" + playing + "}}));";
        handler.post(() -> webView.evaluateJavascript(script, null));
    }
    ...
    private void preloadInstalledApps() {
        ...
        }, "MACH-App-Liste").start();
    }

    private void dispatchInstalledApps() {
        if (!pageReady || webView == null || !installedAppsLoaded) return;
        String script = "window.dispatchEvent(new CustomEvent('mach-installed-apps',{detail:" + installedAppsCache + "}));";
        handler.post(() -> webView.evaluateJavascript(script, null));
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == QR_VERIFY_REQUEST) {
            if (resultCode == RESULT_OK) {
                ...
                if (webView != null) webView.evaluateJavascript("window.dispatchEvent(new CustomEvent('mach-qr-verified'));", null);
        ...
        if (requestCode == NFC_ENROLL_REQUEST) {
            if (resultCode == RESULT_OK && data != null) {
                try {
                    ...
                    String script = "window.dispatchEvent(new CustomEvent('mach-nfc-enrolled',{detail:" + detail + "}));";
                    handler.postDelayed(() -> {
        ...
        try {
            ...
            String script = "window.dispatchEvent(new CustomEvent('mach-custom-sound',{detail:" + detail + "}));";
            if (webView != null) webView.evaluateJavascript(script, null);
        } catch (Exception error) {
    ...
    @Override protected void onDestroy() {
        ...
        stopSoundPreview();
        if (webView != null) { webView.removeJavascriptInterface("MachAndroid"); webView.destroy(); webView = null; }
        super.onDestroy();
    }
}
